...
namespace DataBaseEngine.Repositories
{
    ...
    public class Repository<T> : IRepository<T> where T : class, IEntity
    {
        ...
        private async Task<FirstSnow.Contract.Model.PagedResult<T>> GetAsyncInternal(Filter<T> filter, bool withDeleted, string methodName)
        {
            return await ExecuteAsync(async (context) =>
            {
                ...
                return new FirstSnow.Contract.Model.PagedResult<T>(result, pageCount);
            }, methodName, false);
        }
    ...
    {
        public RepositoryException(Exception ex) : base(ex.Message) { }
        public RepositoryException(string ex) : base(ex) { }
